...
public class ChatNetworkManager : MonoBehaviour, INetworkRunnerCallbacks
{
    ...
    {
        chatRunner = Instantiate(runnerPrefab);
        chatRunner.AddCallbacks(this);

        StartGameResult result = await chatRunner.StartGame(new StartGameArgs
        {
            GameMode = GameMode.Shared,
            SessionName = "GlobalChat",
            PlayerCount = 100
        });

        if (!result.Ok)
        {
            Debug.LogError($"Chat failed: {result.ErrorMessage}");
            return;
        }
